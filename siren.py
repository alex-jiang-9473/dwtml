...
class SirenWithFourier(nn.Module):
    """Fourier + SIREN model.
    ...
    def __init__(self, mapping_size=256, scale=10.0,
                 dim_hidden=256, dim_out=3, num_layers=5,
                 w0=30., w0_initial=30., use_bias=True, final_activation=None):
        super().__init__()
        self.fourier = FourierFeatureEncoding(input_dim=2, mapping_size=mapping_size, scale=scale)
        self.siren = Siren(
            dim_in=2 * mapping_size,
    ...
    def forward(self, coords):  # coords: [N, 2]
        ...
        return self.siren(encoded)

# Our improvement to project the (x,y) coordinates to a higher dimensional space using Fourier based encoding.
class FourierFeatureEncoding(nn.Module):
    ...
    def forward(self, x):
        # x: [N, input_dim]
        x_proj = 2 * pi * x @ self.B.T  # [N, mapping_size]
        return torch.cat([torch.sin(x_proj), torch.cos(x_proj)], dim=-1)  # [N, 2 * mapping_size]
